...
import glob
import torch
import main.size_depth_optimization.utils as utils
import tqdm
import pickle
import numpy as np
...
def run_w_reproj_demo(args, model_path, model_type="large", optimize=True):
    ...
    model, net_w, net_h, resize_mode, normalization, transform = init_network(
        model_type, model_path, device, optimize
    )
    ...
    img_names = [
        c for c in img_names if (".jpg" in c or ".png" in c) and "output." not in c
    ]
    img_names.sort()
