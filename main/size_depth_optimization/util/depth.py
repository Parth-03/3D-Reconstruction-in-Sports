...
def weak_perspective_projection(
    points, translation, camera_center, focal_length=1000, rotation=None
):
    """
    Taken from Coherent Multiperson
    This function computes the perspective projection of a set of points.
...
def project_joints_to_img(
    joints3d, img_size, translation, focal_lenght=1000, return_instrinsics=False
):

    projected_joints_2d = perspective_projection(
        joints3d,
        translation,
...
def weak_project_joints_to_img(joints3d, img_size, translation, focal_lenght=1000):
    ...
    projected_joints_2d = weak_perspective_projection(
        joints3d,
        translation,
        camera_center=img_size / 2,
        focal_length=focal_lenght,
...
"""
jt_arr: array of objects with two keys: "joints_3d" and "translation"
"""


def read_all_joints(jt_arr):
    # these joints are in global order from CRMP so ankels=0, 5
    j3d_all = []
    j3d_translated_all = []
    trans_all = []
