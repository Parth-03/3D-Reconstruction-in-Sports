...
def infer_HMR(fn, model, det_thresh, nms_kernel_size, fov):
    ...
    return humans, img_pil_visu, img_size
...
def generate_3D_scene(humans, model, img_pil_visu):
    ...
    l_mesh = [
        humans[j]["verts_smplx"].detach().cpu().numpy() for j in range(len(humans))
    ]
...
if __name__ == "__main__":
    ...
    HMR_out, img_visu, img_size = infer_HMR(
        fn=img_path, model=HMR_model, det_thresh=0.1, nms_kernel_size=3, fov=60
    )

    # Load Hypergraph model
    _, _, smpl = init(dtype=dtype, note="demo")
    HGraph_model = ModelLoader(
        device=device,
        output=out_folder,
        model=HGraph_model_name,
        pretrain=True,
        pretrain_dir=HGraph_model_dir,
        data_folder=data_folder,
    )
    # Load and preprocess data for Hypergraph Model
    HGraph_dataloader = DatasetLoader(
        dtype=dtype, smpl=smpl, data_folder=data_folder, model=HGraph_model_name
    )
    ...
    Opt_out = run_size_depth_opt(HMR_out, img_size)
